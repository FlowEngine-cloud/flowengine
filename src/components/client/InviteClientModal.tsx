...
import { X, RefreshCw, CheckCircle, Check, Server, MessageSquare } from 'lucide-react';
...
export default function InviteClientModal({
  ...
  const handleInvite = async () => {
    ...
    const config: InviteConfig = {
      ...
      linkedServiceIds: selectedServiceIds.length > 0 ? selectedServiceIds : undefined,
    };
          ...
          <>
            ...
            <div className="px-5 py-5 space-y-5">
              ...
              <div className="space-y-4">
                    ...
                    <div className="space-y-1.5">
                      {agencyServices.map((svc) => (
                        <ItemCheckbox
                          ...
                          label={svc.name}
                          sublabel={svc.phone}
                          onClick={() => toggleItem(svc.id, selectedServiceIds, setSelectedServiceIds)}
                        />
                      ))}
                    </div>
                  </div>
                ...
                {agencyInstances.length === 0 && agencyServices.length === 0 && (
                  <p className="text-white/40 text-sm py-2">
                    No instances or services to assign yet. Client will get portal access.
                  </p>
                )}
              </div>
            </div>
