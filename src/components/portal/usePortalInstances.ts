...
const CACHE_KEY = 'portal-hosting-instances-v3';
...
export function usePortalInstances() {
  ...
  // Tracks the in-flight FlowEngine fetch so it can be aborted on key removal
  const feAbortRef = useRef<AbortController | null>(null);
  ...
  const fetch_ = useCallback(async () => {
    ...
    feAbortRef.current?.abort();
    feAbortRef.current = new AbortController();
    const feSignal = feAbortRef.current.signal;
    supabase.auth.getSession().then(({ data: { session: authSession } }) => {
      if (!authSession?.access_token || feSignal.aborted) return;
      fetch('/api/flowengine/instances', {
        ...
        .then(async (feRes) => {
          if (!feRes.ok) {
            ...
            // Remove any previously-loaded FE instances so stale data isn't shown
            setInstances(prev => {
              const without = prev.filter(i => (i as any).platform !== 'flowengine');
              setCache(without);
              return without;
            });
            return;
        ...
        .catch((err) => {
          if (err?.name === 'AbortError') return; // key was removed — ignore
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      const removed = (e as CustomEvent)?.detail?.removed === true;
      try { sessionStorage.removeItem('portal-hosting-instances-v3'); } catch {}
      // Cancel any in-flight FE fetch so it can't race back and re-add instances
      if (removed) feAbortRef.current?.abort();
      // Always remove FE instances immediately so they don't linger
      setInstances(prev => {
        const without = prev.filter(i => (i as any).platform !== 'flowengine');
        setCache(without);
        return without;
      });
      // Only re-fetch when a new key was saved — skip re-fetch on removal so the
      // background FE call can't race and re-add stale instances before the server
      // settings cache has caught up.
      if (!removed && !authLoading && !teamLoading && user && ownerId) fetch_();
    };
