...
export async function getPortalSettings(): Promise<PortalSettings> {
  ...
  try {
    ...
    for (const [key, envVar] of Object.entries(ENV_FALLBACKS)) {
      ...
      // Use !== undefined (not ??) so an explicit null in the DB means "cleared by user"
      // and does NOT fall through to the env var fallback.
      const dbValue = data?.[key];
      const jsonbValue = oauthCreds[key];
      const envValue = process.env[envVar];
      if (dbValue !== undefined) {
        settings[key] = dbValue;
      } else if (jsonbValue !== undefined) {
        settings[key] = jsonbValue;
      } else {
        settings[key] = envValue ?? null;
      }
    }
