...
import { Plus, ExternalLink, Server, Globe, Link2, AlertTriangle, Cloud } from 'lucide-react';
...
export default function HostingLayout({ children }: { children: React.ReactNode }) {
  ...
  if (instances.length > 0) {
    ...
    // FlowEngine Cloud instances are separate from local/self-hosted instances
    const flowEngineInstances = filtered.filter(i => i.platform === 'flowengine' && !i.deleted_at && i.status !== 'pending_deploy').sort((a, b) => a.instance_name.localeCompare(b.instance_name));
    // Local/self-hosted instances — exclude FlowEngine
    const pendingInstances = filtered.filter(i => i.platform !== 'flowengine' && !i.is_external && (!i.service_type || i.deleted_at || i.status === 'pending_deploy' || !KNOWN_TYPES.has(i.service_type!))).sort((a, b) => a.instance_name.localeCompare(b.instance_name));
    const n8nInstances = filtered.filter(i => i.service_type === 'n8n' && !i.deleted_at && i.status !== 'pending_deploy' && i.platform !== 'flowengine').sort((a, b) => a.instance_name.localeCompare(b.instance_name));
    const openclawInstances = filtered.filter(i => i.service_type === 'openclaw' && !i.deleted_at && i.status !== 'pending_deploy' && i.platform !== 'flowengine').sort((a, b) => a.instance_name.localeCompare(b.instance_name));
    const websiteInstances = filtered.filter(i => i.service_type === 'website' && !i.deleted_at && i.status !== 'pending_deploy' && i.platform !== 'flowengine').sort((a, b) => a.instance_name.localeCompare(b.instance_name));
    const otherInstances = filtered.filter(i => i.service_type === 'other' && !i.deleted_at && i.status !== 'pending_deploy' && i.platform !== 'flowengine').sort((a, b) => a.instance_name.localeCompare(b.instance_name));
    ...
    // FlowEngine Cloud instances — shown first, grouped separately
    if (flowEngineInstances.length > 0) {
      sections.push({ title: 'FlowEngine Cloud', icon: <Cloud className="w-3.5 h-3.5" />, items: flowEngineInstances.map(mapItem) });
    }
