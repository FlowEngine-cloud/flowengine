...
import { FileText, Download, Hash, HelpCircle, X, Lightbulb, Upload, Loader2, CheckCircle, AlertCircle, ChevronDown, Eye, Plus, Table, MessageSquare, Zap, Mail, Bell, Database, Globe, Code, BarChart, Users, Search, Settings, Star, Heart, Bookmark, Tag, Link, Image, Video, Music, Calendar, Clock, Map, Phone, Camera, Mic, Lock, Unlock, Shield, AlertTriangle, Info, Check, Copy, Edit, Trash, Send, Download as DownloadIcon, Upload as UploadIcon } from 'lucide-react';

const LUCIDE_ICON_MAP: Record<string, React.ElementType> = {
  'table': Table, 'message-square': MessageSquare, 'messagesquare': MessageSquare,
  'zap': Zap, 'mail': Mail, 'bell': Bell, 'database': Database, 'globe': Globe,
  'code': Code, 'bar-chart': BarChart, 'barchart': BarChart, 'users': Users,
  'search': Search, 'settings': Settings, 'star': Star, 'heart': Heart,
  'bookmark': Bookmark, 'tag': Tag, 'link': Link, 'image': Image, 'video': Video,
  'music': Music, 'calendar': Calendar, 'clock': Clock, 'map': Map, 'phone': Phone,
  'camera': Camera, 'mic': Mic, 'lock': Lock, 'unlock': Unlock, 'shield': Shield,
  'file-text': FileText, 'filetext': FileText, 'hash': Hash, 'check': Check,
  'copy': Copy, 'edit': Edit, 'trash': Trash, 'send': Send,
  'alert-triangle': AlertTriangle, 'info': Info,
};

function TemplateIcon({ icon }: { icon?: string | null }) {
  if (!icon) return <span>⚡</span>;
  const LucideIcon = LUCIDE_ICON_MAP[icon.toLowerCase()];
  if (LucideIcon) return <LucideIcon className="w-5 h-5 text-purple-300" />;
