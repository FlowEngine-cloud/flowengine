...
function groupByClient(instances: ClientInstance[]): GroupedClient[] {
  ...
  return Array.from(map.entries()).map(([key, insts]) => {
    ...
    const displayName = clientName || (!isPlaceholderEmail(clientEmail) ? clientEmail : null) || 'Unknown client';
...
export default function ClientsLayout({ children }: { children: React.ReactNode }) {
  ...
  const [agencyExternalInstances, setAgencyExternalInstances] = useState<{ id: string; name: string; url: string | null; service_type: string | null }[]>([]);
  ...
  const fetchAgencyExternalInstances = useCallback(async () => {
    if (!session?.access_token) return;
    try {
      const res = await fetch('/api/agency/external-instances', {
        headers: { Authorization: `Bearer ${session.access_token}` },
      });
      if (res.ok) {
        const data = await res.json();
        setAgencyExternalInstances(data.instances || []);
      }
    } catch { /* silently fail */ }
  }, [session?.access_token]);
  ...
  const fetchAgencyServices = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setAgencyServices(data.services || []);
      }
    } catch {
      // silently fail
    }
  ...
  useEffect(() => {
    if (!authLoading && !teamLoading && session) {
      ...
      fetchAgencyExternalInstances();
      fetchAgencyServices();
    }
  }, [authLoading, teamLoading, session, fetchClients, fetchAgencyInstances, fetchAgencyExternalInstances, fetchAgencyServices]);
