...
import {
  ...
  MessageSquare, Plus, Phone, ExternalLink, X,
...
import DeployInstanceModal, { type InstanceConfig, type ConnectInstanceConfig } from '@/components/DeployInstanceModal';
...
export default function ClientDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { grouped, loading, refetch, agencyInstances, agencyExternalInstances, agencyServices, liveStatus, statusLoading } = useClientsContext();
  ...
  const [showAssignExternal, setShowAssignExternal] = useState(false);
  ...
  // Deploy/Add new modals (for "Add New" inside link modals)
  const [showDeployModal, setShowDeployModal] = useState(false);
  const [deployingNewInstance, setDeployingNewInstance] = useState(false);
  ...
  // Real UUID for the client (null if they haven't accepted yet — ni_ / pending: / email slugs)
  const clientRealUserId = (client?.userId && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(client.userId))
    ? client.userId : null;
  // Invite ID for pending/ni_ clients — used to pre-assign instances before they accept
  const pendingInviteId = !clientRealUserId
    ? (client?.instances.find(i => i.invite_id)?.invite_id ?? null)
    : null;
  ...
  const saveClientInfo = async (storedEmail: string) => {
    const trimmedName = clientInfoName.trim();
    const trimmedEmail = clientInfoEmail.trim();
    if (!session?.access_token || !trimmedName) return;
    setClientInfoSaving(true);
    setClientInfoError(null);
    // Only include email in the PATCH if it actually changed
    const emailChanged = trimmedEmail !== storedEmail;
    try {
      ...
      const data = await res.json();
      if (!res.ok) { setClientInfoError(data.error || 'Failed to update'); return; }
      await refetch();
      setEditingClientInfo(false);
    } catch { setClientInfoError('Something went wrong'); } finally {
  ...
  // Direct link by ID from modal (no state intermediary)
  const handleAssignInstanceById = async (instanceId: string) => {
    if (!session?.access_token || !client) return;
    setAssigningInstance(true);
    setAssignInstanceId(instanceId);
    try {
      let res: Response;
      if (clientRealUserId) {
        // Client has a real account — assign directly
        res = await fetch('/api/client/instances', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
          body: JSON.stringify({ instance_id: instanceId, client_user_id: clientRealUserId }),
        });
      } else if (pendingInviteId) {
        // Client hasn't accepted yet — pre-assign via invite
        res = await fetch(`/api/client/invite/${pendingInviteId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
          body: JSON.stringify({ instance_id: instanceId }),
        });
      } else {
        setError('Cannot assign instance: no client invite found.');
  ...
  const handleLinkServiceById = async (serviceId: string, instanceId: string) => {
    if (!session?.access_token) return;
    setLinkingService(true);
    ...
    try {
      const res = await fetch('/api/whatsapp/link', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
        body: JSON.stringify({ whatsappInstanceId: serviceId, portalInstanceId: instanceId }),
      });
      if (res.ok) {
        setShowLinkService(false);
        setLinkServiceId('');
  ...
  // Connect a new instance and auto-assign to this client
  const handleConnectNewInstance = async (config: ConnectInstanceConfig) => {
    if (!session?.access_token) return;
    setDeployingNewInstance(true);
    try {
      const res = await fetch('/api/hosting/connect', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
        body: JSON.stringify({ instanceName: config.name, instanceUrl: config.instanceUrl, apiKey: config.apiKey, serviceType: config.serviceType }),
      });
      const data = await res.json();
      if (data.success) {
        ...
        if (data.instance?.id) {
          if (clientRealUserId) {
            await fetch('/api/client/instances', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
              body: JSON.stringify({ instance_id: data.instance.id, client_user_id: clientRealUserId }),
            });
          } else if (pendingInviteId) {
            await fetch(`/api/client/invite/${pendingInviteId}`, {
              method: 'PATCH',
              headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
              body: JSON.stringify({ instance_id: data.instance.id }),
            });
          }
        }
        refetch();
      ...
      setDeployingNewInstance(false);
    }
  };

  // Cloud deploy — redirect to Stripe checkout
  const handleDeployNewInstance = async (config: InstanceConfig) => {
    if (!session?.access_token) return;
    setDeployingNewInstance(true);
    try {
      const res = await fetch('/api/stripe/create-checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
        body: JSON.stringify({ planType: 'pay_per_instance', billingCycle: config.billingCycle, instanceName: config.name, storageLimit: config.storageSize, cancelUrl: `/portal/clients/${clientUserId}?tab=properties` }),
      });
      if (!res.ok) throw new Error('Checkout failed');
      const data = await res.json();
      if (data.url) window.location.href = data.url;
    } catch { /* handled by modal */ } finally {
      setDeployingNewInstance(false);
    }
  };
  ...
  const handleRevokeAccess = async (instanceId: string) => {
    ...
    try {
      const res = await fetch('/api/client/instances', {
        ...
        body: JSON.stringify({ instance_id: instanceId, user_id: clientRealUserId || clientUserId }),
      });
      const data = await res.json();
      if (!res.ok) {
    ...
    try {
      ...
      for (const inst of realInstances) {
        await fetch('/api/client/instances', {
          ...
          body: JSON.stringify({ instance_id: inst.instance_id, user_id: clientRealUserId || clientUserId }),
        });
      }
  ...
  // Effective client ID for team API calls — real UUID for accepted, pending:inviteId for pending
  const teamClientId = clientRealUserId ?? (pendingInviteId ? `pending:${pendingInviteId}` : null);

  const fetchTeamMembers = useCallback(async () => {
    if (!session?.access_token || !clientUserId || teamLoaded) return;
    if (!teamClientId) { setTeamLoading(false); setTeamLoaded(true); return; }
    setTeamLoading(true);
    try {
      const res = await fetch(`/api/client/team?clientId=${teamClientId}`, {
        headers: { Authorization: `Bearer ${session.access_token}` },
      });
      const data = await res.json();
  ...
  }, [session?.access_token, clientUserId, teamLoaded, teamClientId]);

  const handleTeamInvite = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!teamInviteEmail.trim() || !session?.access_token) return;
    if (!teamClientId) {
      setTeamError('Cannot add team members: no client found.');
      return;
    }
    ...
    try {
      const res = await fetch('/api/client/team', {
        ...
        body: JSON.stringify({ clientId: teamClientId, email: teamInviteEmail.trim(), role: teamInviteRole }),
      });
      const data = await res.json();
      if (!res.ok) {
  ...
  const availableExternalInstances = agencyExternalInstances.filter(i => !assignedInstanceIds.has(i.id));
  ...
  if (activeTab === 'overview') {
    ...
    const rawClientEmail = client.instances[0]?.client_email;
    const storedEmail = isPlaceholderEmail(rawClientEmail) ? '' : (rawClientEmail || '');
          ...
          <div className="bg-gray-900/50 border border-gray-800 rounded-lg p-5">
            {editingClientInfo ? (
              <div className="space-y-3">
                ...
                <div>
                  <label className="block text-sm text-white/60 mb-1.5">Email <span className="text-white/30">(optional)</span></label>
                  <input
                    type="email"
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  <button
                    onClick={() => { setEditingClientInfo(false); setClientInfoName(''); setClientInfoEmail(''); setClientInfoError(null); }}
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-1 min-w-0">
                  <p className="text-base font-semibold text-white truncate">{client.name || client.email}</p>
                  {storedEmail && client.name && (
                    <p className="text-sm text-white/40 truncate">{storedEmail}</p>
                  )}
                </div>
                <button
                  onClick={() => { setClientInfoName(client.name || ''); setClientInfoEmail(storedEmail); setEditingClientInfo(true); }}
                  className="flex items-center gap-1.5 px-3 py-1.5 border border-gray-700 hover:bg-gray-700 text-white/60 rounded-lg text-sm font-medium transition-colors cursor-pointer shrink-0"
                >
  ...
  if (activeTab === 'properties') {
    return (
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-3xl mx-auto p-4 md:p-6 space-y-6">
          ...
          <div id="hosting">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-white/60 uppercase tracking-wide">Hosted Instances</h3>
              <button
          ...
          <div id="external">
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                onClick={() => setShowAssignExternal(true)}
                className="flex items-center gap-1.5 px-3 py-1.5 border border-gray-700 hover:bg-gray-700 text-white/60 rounded-lg text-sm font-medium transition-colors cursor-pointer"
              >
                <Plus className="w-3.5 h-3.5" />
            ...
            <div className="relative w-full max-w-md bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
              ...
              {availableInstances.length === 0 ? (
                <div className="p-8 flex flex-col items-center text-center">
                  <Server className="w-8 h-8 text-white/20 mb-2" />
                  <p className="text-sm text-white/40">All instances are already linked, or none exist yet.</p>
                </div>
              ) : (
                <div className="max-h-72 overflow-y-auto divide-y divide-gray-800/50">
                  {availableInstances.map(inst => (
                    <button
                      key={inst.id}
                      onClick={() => handleAssignInstanceById(inst.id)}
                      disabled={assigningInstance}
                      className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-900/60 transition-colors cursor-pointer text-left disabled:opacity-50"
                    >
                      <Server className="w-4 h-4 text-white/40 shrink-0" />
                      <span className="text-sm text-white flex-1">{inst.name}</span>
                      {assigningInstance && assignInstanceId === inst.id && <Loader2 className="w-4 h-4 text-white/40 animate-spin shrink-0" />}
                    </button>
                  ))}
                </div>
              )}
              <div className="px-4 py-3 border-t border-gray-800">
                <button
                  onClick={() => { setShowAssignInstance(false); setShowDeployModal(true); }}
                  className="w-full flex items-center justify-center gap-1.5 px-4 py-2.5 border border-dashed border-gray-700 hover:bg-gray-800/50 text-white/40 hover:text-white rounded-lg text-sm font-medium transition-colors cursor-pointer"
                >
                  <Plus className="w-3.5 h-3.5" />
                  Add New Instance
                </button>
        ...
        {showAssignExternal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowAssignExternal(false)}>
            ...
            <div className="relative w-full max-w-md bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
              ...
              {availableExternalInstances.length === 0 ? (
                <div className="p-8 flex flex-col items-center text-center">
                  <Server className="w-8 h-8 text-white/20 mb-2" />
                  <p className="text-sm text-white/40">All external instances are already linked, or none exist yet.</p>
                </div>
              ) : (
                <div className="max-h-72 overflow-y-auto divide-y divide-gray-800/50">
                  {availableExternalInstances.map(inst => (
                    <button
                      key={inst.id}
                      onClick={() => handleAssignInstanceById(inst.id)}
                      disabled={assigningInstance}
                      className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-800/60 transition-colors cursor-pointer text-left disabled:opacity-50"
                    >
                      <Server className="w-4 h-4 text-white/40 shrink-0" />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-white truncate">{inst.name}</p>
                        {inst.url && <p className="text-xs text-white/40 truncate">{inst.url.replace('https://', '')}</p>}
                      </div>
                      {inst.service_type && (
                        <span className="text-xs text-white/40 border border-gray-700 rounded px-1.5 py-0.5 shrink-0 capitalize">{inst.service_type}</span>
                      )}
                      {assigningInstance && assignInstanceId === inst.id && <Loader2 className="w-4 h-4 text-white/40 animate-spin shrink-0" />}
                    </button>
                  ))}
              ...
              <div className="px-4 py-3 border-t border-gray-800">
                <button
                  onClick={() => { setShowAssignExternal(false); setShowDeployModal(true); }}
                  className="w-full flex items-center justify-center gap-1.5 px-4 py-2.5 border border-dashed border-gray-700 hover:bg-gray-800/50 text-white/40 hover:text-white rounded-lg text-sm font-medium transition-colors cursor-pointer"
                >
                  <Plus className="w-3.5 h-3.5" />
                  Add New Instance
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => { setShowLinkService(false); setLinkServiceId(''); setLinkServiceInstanceId(''); }}>
            ...
            <div className="relative w-full max-w-md bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
              ...
              {realInstances.length === 0 ? (
                <div className="p-8 flex flex-col items-center text-center">
                  <MessageSquare className="w-8 h-8 text-white/20 mb-2" />
                  <p className="text-sm text-white/40">Link a hosted instance first before adding services.</p>
                </div>
              ) : (
                <>
                  {realInstances.length > 1 && (
                    <div className="px-4 py-3 border-b border-gray-800">
                      <p className="text-xs text-white/40 mb-2">Link service to instance:</p>
                      <SearchableSelect
                  ...
                  {agencyServices.length === 0 ? (
                    <div className="p-8 flex flex-col items-center text-center">
                      <MessageSquare className="w-8 h-8 text-white/20 mb-2" />
                      <p className="text-sm text-white/40">No services available. Add a new service below.</p>
                    </div>
                  ) : (
                    <div className="max-h-64 overflow-y-auto divide-y divide-gray-800/50">
                      {agencyServices.map(s => {
                        const instId = realInstances.length === 1 ? realInstances[0].instance_id : linkServiceInstanceId;
                        return (
                          <button
                            key={s.id}
                            onClick={() => { if (instId) handleLinkServiceById(s.id, instId); }}
                          ...
                          >
                            ...
                            {linkingService && linkServiceId === s.id && <Loader2 className="w-4 h-4 text-white/40 animate-spin shrink-0" />}
                          </button>
                        );
                      })}
                    </div>
              ...
              <div className="px-4 py-3 border-t border-gray-800">
                <button
                  onClick={() => { setShowLinkService(false); setShowAddServiceModal(true); }}
                  className="w-full flex items-center justify-center gap-1.5 px-4 py-2.5 border border-dashed border-gray-700 hover:bg-gray-800/50 text-white/40 hover:text-white rounded-lg text-sm font-medium transition-colors cursor-pointer"
                >
                  <Plus className="w-3.5 h-3.5" />
                  Add New Service
        ...
        {/* ── Deploy Instance Modal (Add New Instance) ────────────────────────── */}
        <DeployInstanceModal
          isOpen={showDeployModal}
          onClose={() => setShowDeployModal(false)}
          onDeploy={handleDeployNewInstance}
          onConnectInstance={handleConnectNewInstance}
          existingInstanceCount={agencyInstances.length}
          isDeploying={deployingNewInstance}
          accessToken={session?.access_token}
          onSuccess={() => { refetch(); }}
        />

        {/* ── Add Service Modal (Add New Service) ─────────────────────────────── */}
        <AddServiceModal
