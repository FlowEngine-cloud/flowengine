...
function AcceptAccessContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/invite/accept-access?token=${encodeURIComponent(token)}`)
      .then(r => r.json())
      .then(data => {
  ...
  const handleAccept = async () => {
    ...
    setAcceptError(null);
    try {
      const res = await fetch('/api/invite/accept-access', {
        method: 'POST',
        headers: {
...
export default function AcceptAccessPage() {
  return (
    <Suspense fallback={
      <div className="flex items-center justify-center min-h-screen bg-black">
        <Loader2 className="h-6 w-6 animate-spin text-white/40" />
