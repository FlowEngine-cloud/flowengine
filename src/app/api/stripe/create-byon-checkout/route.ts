...
export async function POST(req: NextRequest) {
  try {
    ...
    // Resolve team context — team members bill under the owner's account
    ...
    // Check for existing pending invite to this email from this owner
    ...
    if (!customerId) {
      const customer = await getStripe().customers.create({
        ...
        metadata: { supabase_user_id: effectiveUserId },
      });
