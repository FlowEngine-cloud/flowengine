...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: invite, error: fetchErr } = await supabaseAdmin
      .from('team_members')
      .select('id, email, owner_id, status, invited_at')
      .eq('token', token)
      .maybeSingle();
    ...
    // Expire after 7 days (use invited_at as source of truth, compatible with older DB schema)
    if (invite.invited_at && new Date(invite.invited_at).getTime() + 7 * 24 * 60 * 60 * 1000 < Date.now()) {
