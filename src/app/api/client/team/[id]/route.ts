...
async function verifyAgencyClientRelation(agencyOwnerId: string, clientUserId: string): Promise<boolean> {
  const [{ data: invite }, { data: ci }, { data: inst }] = await Promise.all([
    supabaseAdmin.from('client_invites').select('id').eq('accepted_by', clientUserId).eq('invited_by', agencyOwnerId).maybeSingle(),
    supabaseAdmin.from('client_instances').select('id').eq('user_id', clientUserId).eq('invited_by', agencyOwnerId).maybeSingle(),
    supabaseAdmin.from('pay_per_instance_deployments').select('id').eq('user_id', agencyOwnerId).eq('client_id', clientUserId).limit(1).maybeSingle(),
  ]);
...
async function verifyClientHasAgency(clientUserId: string): Promise<boolean> {
  const [{ data: ci }, { data: inv }, { data: inst }] = await Promise.all([
    supabaseAdmin.from('client_instances').select('id').eq('user_id', clientUserId).limit(1).maybeSingle(),
    supabaseAdmin.from('client_invites').select('id').eq('accepted_by', clientUserId).limit(1).maybeSingle(),
    supabaseAdmin.from('pay_per_instance_deployments').select('id').eq('client_id', clientUserId).limit(1).maybeSingle(),
  ]);
  return !!(ci || inv || inst);
}
