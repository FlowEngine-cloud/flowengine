...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      emailSent = false;
      // Non-fatal: invite is created, user can share the invite URL manually
    }

    return NextResponse.json({
      success: true,
      message: emailSent ? `Invitation sent to ${email}` : `Client added (email delivery failed — share this link manually: ${inviteUrl})`,
      inviteId: invite.id,
