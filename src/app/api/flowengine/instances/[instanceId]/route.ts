...
export async function PATCH(
  ...
  try {
    ...
    if (!authorized) return NextResponse.json({ error: 'Access denied' }, { status: 403 });
    if (!canWrite(role)) return NextResponse.json({ error: 'Your role does not allow modifying instances' }, { status: 403 });
...
export async function DELETE(
  ...
  try {
    ...
    const { authorized, role } = await verifyFlowEngineAccess(supabaseAdmin, user.id);
    if (!authorized) return NextResponse.json({ error: 'Access denied' }, { status: 403 });
    if (!canWrite(role)) return NextResponse.json({ error: 'Your role does not allow deleting instances' }, { status: 403 });
