...
// Build a chainable mock chain: from().select().order().limit().maybeSingle()
...
describe('PATCH /api/settings/portal', () => {
  ...
  it('returns 403 for non-admin team member', async () => {
    const { getEffectiveOwnerId, canManageBilling } = await import('@/lib/teamUtils');
    (getEffectiveOwnerId as any).mockResolvedValueOnce({ ownerId: 'owner-999', role: 'member', isTeamMember: true });
    (canManageBilling as any).mockReturnValueOnce(false);
    mockAuth('user-123');
