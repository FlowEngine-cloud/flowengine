 ...
 * Returns the authenticated user's n8n instances.
...
export async function GET(req: NextRequest) {
  ...
  const { data: instances, error } = await supabaseAdmin
    ...
    .select('id, instance_name, instance_url, status, created_at, updated_at')
    .eq('user_id', userId)
    .is('deleted_at', null)
    .order('created_at', { ascending: false });

  if (error) {
    return NextResponse.json({ success: false, error: 'Database error', message: 'Failed to fetch instances' }, { status: 500 });
  }
